...
class ModelEvaluator:
    ...
    def error_analysis(
        self, y_true: np.ndarray, y_pred: np.ndarray, confidence: np.ndarray = None
    ) -> pd.DataFrame:
        """
    ...
    def confusion_patterns(
        self, y_true: np.ndarray, y_pred: np.ndarray, top_n: int = 10
    ) -> pd.DataFrame:
        """
        ...
        patterns = pd.DataFrame(
            {"true_label": y_true_errors, "predicted_label": y_pred_errors}
        )

        pattern_counts = patterns.groupby(["true_label", "predicted_label"]).size()
        pattern_counts = pattern_counts.reset_index(name="count")
        pattern_counts = pattern_counts.sort_values("count", ascending=False).head(
            top_n
        )
    ...
    def critical_errors(
        self, y_true: np.ndarray, y_pred: np.ndarray, critical_classes: List[str] = None
    ) -> Dict[str, int]:
        """
        ...
        missed = (y_true[critical_mask] != y_pred[critical_mask]) & (
            y_pred[critical_mask] == "normal"
        )
