...
class EvaluationVisualizer:
    ...
    def plot_error_analysis(
        ...
        axes[0].bar(
            error_df["Class"], error_df["Error Rate (%)"], color="coral", alpha=0.7
        )
        ...
        if len(confusion_patterns) > 0:
            ...
            axes[1].set_title(
                "Top Misclassification Patterns", fontweight="bold", fontsize=14
            )
            axes[1].invert_yaxis()
    ...
    def plot_feature_importance(
        ...
        ax.set_title(
            f"Top {top_n} Most Important Features", fontweight="bold", fontsize=14
        )
        ax.invert_yaxis()
