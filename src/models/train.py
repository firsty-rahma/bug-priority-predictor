...
class ModelTrainer:
    ...
    def create_pipeline(
        ...
        if use_smote:
            steps.append(
                (
                    "smote",
                    SMOTE(random_state=self.random_state, k_neighbors=smote_neighbors),
                )
            )
    ...
    def train_with_cv(
        ...
        results = {"mean": scores.mean(), "std": scores.std(), "scores": scores}
    ...
    def evaluate(
        self, model: Any, X_test: pd.DataFrame, y_test: np.ndarray
    ) -> Dict[str, Any]:
        """
    ...
    def save_model(
        self, model: Any, filepath: Path, include_encoder: bool = True
    ) -> None:
        """
