...
def main():
    ...
    for model_name, config in models_to_tune.items():
        ...
        # Create pipeline
        feature_combiner = FeatureCombiner(max_features=1000, min_df=2, max_df=0.8)

        pipeline = trainer.create_pipeline(
            feature_combiner, config["classifier"], use_smote=True
        )

        # Hyperparameter tuning
        grid_search = trainer.hyperparameter_tuning(
            pipeline, config["param_grid"], X_train, y_train, cv_folds=CV_FOLDS
        )
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(classification_report(eval_results["y_test"], eval_results["y_pred"]))
    ...
    visualizer = EvaluationVisualizer(
        class_labels=sorted(trainer.label_encoder.classes_)
    )
