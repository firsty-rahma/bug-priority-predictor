...
from data.loader import clean_raw_data, load_raw_data, save_data
from utils.config import CLEANED_DATA_PATH, FIGURES_DIR, RAW_DATA_PATH
...
def explore_data(data: pd.DataFrame) -> None:
    ...
    logger.info("\nPercentages:")
    percentages = (data["severity_category"].value_counts(normalize=True) * 100).round(
        2
    )
    ...
    if "severity_code" in data.columns:
        ...
        logger.info(
            "→ Decision: Will drop severity_code column and use severity_category as the target variable"
        )
...
def visualize_distributions(data: pd.DataFrame, output_dir: Path) -> None:
    ...
    # Percentage plot
    severity_pct = (
        data["severity_category"].value_counts(normalize=True) * 100
    ).sort_index()
