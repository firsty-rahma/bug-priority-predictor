...
class TestIntegration:
    ...
    def test_full_pipeline_flow(self, sample_bug_data, temp_directory):
        ...
        data_processed = preprocessor.preprocess_dataframe(
            sample_bug_data, show_progress=False
        )
        ...
        X = data_processed[
            ["text_processed", "component_name", "product_name", "text_length"]
        ]
        ...
        feature_combiner = FeatureCombiner(max_features=10, min_df=1, max_df=1.0)
        classifier = RandomForestClassifier(n_estimators=10, random_state=42)
        pipeline = trainer.create_pipeline(
            feature_combiner, classifier, use_smote=False  # Disabled for test
        )
    ...
    def test_preprocessing_to_features(self, sample_bug_data):
        ...
        preprocessor = TextPreprocessor(
            custom_stopwords=["test", "bug"], use_pos_lemmatization=False
        )
        data_processed = preprocessor.preprocess_dataframe(
            sample_bug_data, show_progress=False
        )
        ...
        combiner = FeatureCombiner(max_features=20, min_df=1, max_df=1.0)
        combiner.fit(X)
